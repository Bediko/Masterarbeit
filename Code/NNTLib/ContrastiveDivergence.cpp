...
void ContrastiveDivergence::GibbsSampling(int gibbssteps, int d_i) {
	...
	for (int g = 0; g < gibbssteps; g++) {
		...
		if (gibbssteps == 1 || g == gibbssteps - 1) {
...
void ContrastiveDivergence::UpdateHiddenUnitsWithSampling() {
...
void ContrastiveDivergence::UpdateHiddenUnitsWithoutSampling() {

	for ( int j = 0; j < top->NeuronCount - 1; j++) {
		double sum = 0.0;
		for (int i = 0; i < bottom->NeuronCount - 1; i++) {
			sum += bottom->Neurons[j].Output * top->Neurons[j].Weights[i];
		}
