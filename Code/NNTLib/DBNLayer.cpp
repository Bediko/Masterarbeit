...
void DBNLayer::Forwardweightsinit(int Neuronsdown, DBNLayer* Layerup) {
	for (int i = 0; i < Neuronsdown; i++) {
		...
		for (int j = 0; j < Layerup->NeuronCount; j++) { //-1 da keine Gewichte zum Bias
			Neurons[i].ForwardWeights[j] = &Layerup->Neurons[j].Weights[i];
		}
	}
	Neurons[Neuronsdown].ForwardWeightCount = Layerup->NeuronCount-1;
	Neurons[Neuronsdown].ForwardWeights = new double*[Neurons[Neuronsdown].ForwardWeightCount];
	for (int j = 0; j < Layerup->NeuronCount-1; j++) { //-1 da keine Gewichte zum Bias
		Neurons[Neuronsdown].ForwardWeights[j] = &Layerup->Neurons[j].Weights[Neuronsdown];
	}
